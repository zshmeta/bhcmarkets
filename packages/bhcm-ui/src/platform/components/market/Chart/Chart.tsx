...
import { useWatchlistStore, selectSelectedSymbol, selectCategories } from '@repo/sdk';
import { useAutomationStore } from '@repo/sdk';
import { useTradingStore } from '@repo/sdk';
import { SymbolSelector, type Category as SymbolSelectorCategory } from '../../core/SymbolSelector';
import { useIsMobile } from '../../hooks/useMediaQuery';
import { useI18n } from '../../i18n';
import { Icons } from '../Icons';
import {
  Container, Toolbar, ToolbarRight, ToolbarScrollArea,
...
type MainSeriesApi = ISeriesApi<'Candlestick'> | ISeriesApi<'Line'>;
type VolumeSeriesApi = ISeriesApi<'Histogram'>;
type IndicatorSeriesApi = ISeriesApi<'Line'>;
...
const Chart = () => {
  ...
  const indicatorSeriesRef = useRef<Map<string, IndicatorSeriesApi>>(new Map());
  const klinesRef = useRef<KlineData[]>([]);

  const selectedSymbol = useWatchlistStore(selectSelectedSymbol);
  const watchlistCategories = useWatchlistStore(selectCategories);
  ...
  const symbolSelectorCategories = useMemo<SymbolSelectorCategory[]>(() => {
    const flatten = (cats: typeof watchlistCategories, parentLabel?: string): SymbolSelectorCategory[] => {
      return cats.flatMap((cat) => {
        const label = parentLabel ? `${parentLabel} / ${cat.label}` : cat.label;
        const current: SymbolSelectorCategory = {
          id: cat.id,
          label,
          items: (cat.symbols || []).map((s) => ({
            id: s.symbol,
            symbol: s.symbol,
            description: s.baseAsset && s.quoteAsset ? `${s.baseAsset}/${s.quoteAsset}` : undefined,
          })),
        };

        const children = cat.children?.length ? flatten(cat.children, label) : [];
        return [current, ...children];
      });
    };

    return flatten(watchlistCategories);
  }, [watchlistCategories]);
  ...
  const fetchKlines = useCallback(async (symbol: string, interval: string, retryCount = 0) => {
    ...
    try {
      ...
      const formattedData: KlineData[] = (data.candles as MarketCandle[]).map((c) => {
        return {
  ...
  useEffect(() => {
    if (selectedSymbol) {
      console.log('[Chart] Fetching klines for', selectedSymbol, INTERVAL_MAP[timeRange]);
      fetchKlines(selectedSymbol, INTERVAL_MAP[timeRange]);
    } else {
      console.log('[Chart] No selectedSymbol, skipping fetch');
    }
    ...
    const rafId = requestAnimationFrame(() => {
      ...
      chart.subscribeCrosshairMove((param) => {
        ...
        if (data && 'open' in data) {
          ...
          const matchingKline = klinesRef.current.find(k => k.time === param.time);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (klines.length > 0 && mainSeriesRef.current) {
        if (chartType === 'line') {
  ...
  }, [activeIndicators, chartReady, klines]);
  ...
  useEffect(() => {
    ...
    if (activeIndicators.has('MA')) {
      try {
        ...
        periods.forEach((period, i) => {
          ...
          const series = chart.addSeries(LineSeries, { color: colors[i], lineWidth: 1, priceLineVisible: false, lastValueVisible: false });
          series.setData(data);
    ...
    if (activeIndicators.has('BOLL')) {
      try {
        ...
        ].forEach(({ data, color, key, style }) => {
          const series = chart.addSeries(LineSeries, { color, lineWidth: 1, lineStyle: style, priceLineVisible: false, lastValueVisible: false });
          series.setData(data);
  ...
  return (
    <Container $fullscreen={isFullscreen} className="card">
      <Toolbar>
        <SymbolSelector
          categories={symbolSelectorCategories}
        />
        <ToolbarScrollArea>
