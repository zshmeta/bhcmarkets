...
import type { PaperOrder, OrderSide, OrderType } from '@repo/sdk';
	...
	createOrder: async (input) => {
		...
		if (state.isPaperTrading) {
			...
			if (input.type === 'market') {
				const filled = { ...order, status: 'filled', filledQty: input.quantity, updatedAt: Date.now() } as PaperOrder;
	...
	cancelOrder: (clientOrderId) => {
		set((state) => ({
			orders: state.orders.map((o) =>
				o.clientOrderId === clientOrderId ? ({ ...o, status: 'cancelled', updatedAt: Date.now() } as PaperOrder) : o
			),
		}));
	},
