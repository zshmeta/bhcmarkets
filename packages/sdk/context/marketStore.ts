...
import type { DataConfidenceLevel, Level2BookData, MarketMetrics, MarketTrade as Trade } from '@repo/sdk'
import type { NetworkEvent } from '@repo/sdk';
...
const nowTrade = (symbol: string, mid: number): Trade => {
	const price = randomAround(mid, 5).toFixed(2);
	const quantity = (0.001 + Math.random() * 0.05).toFixed(4);
	return {
		id: generateUUID(),
		symbol,
		price,
		quantity,
		quoteQty: (Number(price) * Number(quantity)).toFixed(2),
		time: Date.now(),
...
export const useMarketStore = create<MarketState>((set, get) => ({
	...
	subscribe: (symbol) => {
		...
		if (currentState.isLiveMode) {
			...
			try {
				// Use process.env for Node/Mobile/Metro compatibility
				const wsUrl = (typeof process !== 'undefined' && process.env?.VITE_MARKET_DATA_WS_URL) ||
					'ws://localhost:6060/ws';
				socket = new WebSocket(wsUrl);
				// socket = new WebSocket(wsUrl); (already assigned above)

				// Removed broken event listeners block that used 'this' incorrectly
